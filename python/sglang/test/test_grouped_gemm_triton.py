...
class TestGroupedGemmMaskedTriton(unittest.TestCase):
    ...
    def _create_deterministic_inputs(self):
        ...
        # 注意：现在使用原始三维形状，不需要重塑
        self.a = torch.zeros(self.num_experts, self.seq_len, self.hidden_size, device=self.device)
        ...
        self.b = torch.zeros(self.num_experts, self.intermediate_size, self.hidden_size, device=self.device)
        for i in range(self.num_experts):
        ...
        # 创建每个专家处理的有效token数
        self.masked_m = torch.tensor([1, 2], device=self.device)  # 每个专家处理的有效token数
        ...
        self.c = torch.zeros(self.num_experts, self.seq_len, self.intermediate_size,
