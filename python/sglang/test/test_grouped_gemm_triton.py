...
class TestGroupedGemmTriton(unittest.TestCase):
    ...
    def _create_deterministic_inputs(self):
        ...
        self.b = torch.zeros(
            self.batch_size, self.out_size, self.hidden_size, device=self.device
        )
        ...
        self.weight_indices = torch.arange(
        ...
        # 预分配输出张量
        self.c = torch.zeros(
            total_seqs, self.out_size, device=self.device, dtype=torch.bfloat16
        )
    ...
    def test_different_sequence_lengths(self):
        ...
        # 创建输入
        a = torch.ones(
            total_seqs, self.hidden_size, device=self.device, dtype=torch.bfloat16
        )
        ...
        # 验证结果
        rel_diff = torch.max(torch.abs(result - reference)) / torch.max(
            torch.abs(reference)
        )
        self.assertTrue(rel_diff < 1e-2, f"Relative difference too large: {rel_diff}")
    ...
    def test_edge_cases(self):
        ...
        # 验证单专家输出
        self.assertEqual(
            single_batch_result.shape, (self.seq_per_batch[0], self.out_size)
        )
...
class TestGroupedGemmMaskedTriton(unittest.TestCase):
    ...
    def _create_deterministic_inputs(self):
        ...
        # 注意：现在使用原始三维形状，不需要重塑
        self.a = torch.empty(
            self.num_experts, self.seq_len, self.hidden_size, device=self.device
        )
    ...
    def test_basic_functionality(self):
        ...
        for i in range(self.num_experts):
            ...
            for j in range(self.masked_m[i].item()):
                ...
                expert_output = torch.matmul(
                    ...
                    self.b[i]
                    .transpose(0, 1)
                    .to(torch.float32),  # 转置为[hidden_size, intermediate_size]
                ).to(torch.bfloat16)
        ...
        valid_mask = torch.zeros(
            (self.num_experts, self.seq_len), dtype=torch.bool, device=self.device
        )
        for i in range(self.num_experts):
    ...
    def test_without_weight_indices(self):
        ...
        result = grouped_gemm_masked_triton(
            a=self.a, b=self.b, c=None, masked_m=self.masked_m, c_dtype=torch.bfloat16
        )
        ...
        # 计算误差
        rel_diff = torch.max(torch.abs(result_valid - reference_valid)) / torch.max(
            torch.abs(reference_valid)
        )
    ...
    def test_with_padding(self):
        ...
        result = grouped_gemm_masked_triton(
            a=a_padded, b=self.b, c=None, masked_m=self.masked_m, c_dtype=torch.bfloat16
        )
    ...
    def test_extreme_padding(self):
        ...
        if valid_mask.any():
            ...
            self.assertTrue(
                rel_diff < 1e-2, f"Relative difference too large: {rel_diff}"
            )
        else:
    ...
    def test_custom_weight_indices(self):
        ...
        custom_weight_indices = torch.tensor(
            [2, 0], device=self.device
        )  # 修改为与专家数量匹配

        # 使用 grouped_gemm_masked_triton 计算
        result = grouped_gemm_masked_triton(
            a=self.a, b=custom_b, c=None, masked_m=self.masked_m, c_dtype=torch.bfloat16
        )
    ...
    def test_performance(self):
        ...
        print(
            f"Performance test - Triton: {triton_time:.3f} ms, PyTorch: {pytorch_time:.3f} ms, Speedup: {speedup:.2f}x"
        )
