...
class RoPETestGPU(unittest.TestCase):
    def setUp(self):
        try:
    ...
    def test_rope(self):
        ...
        for is_neox_style in [False, True]:
            ...
            torch_output = rope_torch.forward_native(positions_torch, x_q_torch, x_k_torch)
            q_torch_output_float32 = torch_output[0].cpu().to(torch.float32)
            k_torch_output_float32 = torch_output[1].cpu().to(torch.float32)
