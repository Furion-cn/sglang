...
def create_sample_qwen3_moe_model(output_dir):
    ...
    try:
        mock_weights = {
            'model': {
                'embed_tokens': {
                    'kernel': np.random.randn(config['vocab_size'], config['hidden_size']).astype(np.float32)
                },
                'layers': {},
                'norm': {
            ...
            'lm_head': {
                'kernel': np.random.randn(config['hidden_size'], config['vocab_size']).astype(np.float32)
            }
        }
        ...
        for i in range(config['num_hidden_layers']):
            layer_weights = {
                ...
                'self_attn': {
                    'q_proj': {
                        'kernel': np.random.randn(config['hidden_size'], config['hidden_size']).astype(np.float32)
                    },
                    'k_proj': {
                        'kernel': np.random.randn(config['hidden_size'], config['hidden_size']).astype(np.float32)
                    },
            ...
            if i not in config['mlp_only_layers']:
                ...
                moe_weights = {
                    'gate': {
                        'kernel': np.random.randn(config['num_experts'], config['hidden_size']).astype(np.float32)
                    },
                    'experts': {}
                }
                ...
                for expert_idx in range(config['num_experts']):
                    expert_weights = {
                        'gate_proj': {
                            'kernel': np.random.randn(config['moe_intermediate_size'], config['hidden_size']).astype(np.float32)
                        },
                        'up_proj': {
                            'kernel': np.random.randn(config['moe_intermediate_size'], config['hidden_size']).astype(np.float32)
                        },
                        'down_proj': {
                            'kernel': np.random.randn(config['hidden_size'], config['moe_intermediate_size']).astype(np.float32)
                        }
                    }
                ...
                layer_weights['mlp'] = {
                    'gate_proj': {
                        'kernel': np.random.randn(config['intermediate_size'], config['hidden_size']).astype(np.float32)
                    },
                    'up_proj': {
                        'kernel': np.random.randn(config['intermediate_size'], config['hidden_size']).astype(np.float32)
                    },
                    'down_proj': {
                        'kernel': np.random.randn(config['hidden_size'], config['intermediate_size']).astype(np.float32)
                    }
                }
        ...
        print(f"  - MoE config: {config['num_experts']} experts, {config['num_experts_per_tok']} experts per token")
...
def main():
    ...
    parser.add_argument(
        "--check-jax",
        action="store_true",
        help="Check JAX dependencies and exit"
    )
