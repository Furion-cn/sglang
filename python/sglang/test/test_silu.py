...
class TestSiluAndMulMaskedFwd(unittest.TestCase):
    ...
    def test_basic_functionality(self):
        ...
        # 创建输入张量
        input = torch.randn(
            expert_num, token_num, hidden_dim, dtype=torch.bfloat16, device=self.device
        )
        # 创建输出张量
        output = torch.empty(
            expert_num,
        ...
        self.assertTrue(
            torch.allclose(output_cpu, expected_output, rtol=1e-2, atol=1e-2)
        )
    ...
    def test_different_expert_tokens(self):
        ...
        input = torch.randn(
            expert_num, max_tokens, hidden_dim, dtype=torch.bfloat16, device=self.device
        )
        output = torch.empty(
            expert_num,
            max_tokens,
        ...
        for e, token_num in enumerate(token_nums):
            ...
            for t in range(token_num):
                ...
                expected = gate * up
                self.assertTrue(
                    torch.allclose(output_cpu[e, t], expected, rtol=1e-2, atol=1e-2)
                )

            # 验证填充token的输出（应该为0）
            for t in range(token_num, max_tokens):
                self.assertTrue(
                    torch.allclose(output_cpu[e, t], torch.zeros_like(output_cpu[e, t]))
                )
    ...
    def test_input_validation(self):
        ...
        # 测试错误的hidden_dim
        input = torch.randn(
            expert_num, token_num, 255, dtype=torch.bfloat16, device=self.device
        )
        output = torch.empty(
            expert_num, token_num, 127, dtype=torch.bfloat16, device=self.device
        )
