...
from sglang.srt.distributed.kv_transfer.kv_pipe.base import KVTransferAgent

logger = logging.getLogger(__name__)
NONE_INT = -150886311


class RDMA_OP(enum.Enum):
    READ = 0
    WRITE = 1
...
class MooncakeTransferEngine:
    ...
    def send_bytes(self, remote_url: str, dst_ptr: int, user_data: bytes) -> None:
        ...
        # 3. write the data to the target remote buffer address
        self.transfer_sync(remote_url, src_ptr, dst_ptr, length, RDMA_OP.WRITE)
        # 4. send ACK to the peer
        self.receiver_ack.send_pyobj((remote_url, b'ACK'))
        # 5. free the buffer
        self.free_managed_buffer(src_ptr, length)

    def recv_bytes(self,remote_url: str, rid: str, length: int) -> bytes:
        """Receive bytes from the remote process."""
        # 1. allocate a buffer
        dst_ptr = self.allocate_managed_buffer(length)
        # 2. send control-plan message to the peer
        self.sender_socket.send_pyobj((remote_url, (self.local_url, dst_ptr, rid, length)))
        # 3. wait for the peer to write the data
        ack = self.sender_ack.recv_pyobj()
        if ack != b'ACK':
            logger.error("Failed to receive ACK from the receiver")
...
class MooncakePipe(KVTransferAgent):
    ...
    def _send_impl(self, dst_prt: int, tensor: torch.Tensor) -> None:
        """Implement the tensor sending logic."""
        value_bytes = pickle.dumps(tensor)
        self.transfer_engine.send_bytes(dst_prt, value_bytes)

    def _recv_impl(self, socket_addr: str, rid: str, length: int) -> torch.Tensor:
        """Implement the tensor receiving logic."""
        data = self.transfer_engine.recv_bytes(socket_addr, rid, length)
    ...
    def recv_kv_cache(self, socket_addr: str, rid: str, length: int) -> Optional[torch.Tensor]:
        ...
        if self.transport_thread is None:
            self.transport_thread = ThreadPoolExecutor(max_workers=1)
        tensor = self.transport_thread.submit(self._recv_impl, socket_addr, rid, length).result()
