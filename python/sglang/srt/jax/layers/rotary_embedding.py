...
from sglang.srt.jax.max_logging import max_logging
...
class LLaMARotaryEmbedding(RotaryEmbedding):
    ...
    def setup(self) -> None:
        ...
        if self.use_scale:
            timescale = 1.0 / \
                jax.vmap(self._apply_scaling_factor)(1.0 / timescale)
...
class YarnRotaryEmbedding(nn.Module):
    ...
    def setup(self) -> None:
        ...
        freqs = 1.0 / (self.rope_theta ** (2.0 * jnp.arange(0,
                       half_dim, dtype=jnp.float32) / self.embedding_dims))
        ...
        # shape [max_position_embeddings, half_dim]
        self.freqs_cis = jnp.exp(1j * freqs)
    ...
    def _find_correction_range(self, low_rot: float, high_rot: float, dim: int, base: float, max_position_embeddings: int):
        ...
        """
        low = math.floor(self._find_correction_dim(
            low_rot, dim, base, max_position_embeddings))
        high = math.ceil(self._find_correction_dim(
    ...
    def _linear_ramp_factor(self, min_val: float, max_val: float, dim: int) -> Array:
        ...
        linear_func = (jnp.arange(dim, dtype=jnp.float32) -
    ...
    def __call__(self, inputs: Array, position: Optional[Array] = None) -> Array:
        ...
        # shape: [B, S, half_dim]
        freqs = jnp.take(self.freqs_cis, position, axis=0)
        ...
        # shape: [B, S, N, 2, half_dim]
        rotated_real = jnp.stack(
            [jnp.real(rotated), jnp.imag(rotated)], axis=-2)
...
class PositionalEmbedding(nn.Module):
    ...
    def __call__(
        ...
        inv_timescales = jnp.exp(jnp.arange(
            num_timescales, dtype=jnp.float32) * -log_timescale_increment)
...
class LlamaVisionRotaryEmbedding(nn.Module):
    ...
    def setup(self):
        ...
        rope_freq = 1.0 / (self.rope_theta ** (jnp.arange(0, freq_dim, 2)
                           [: (freq_dim // 2)].astype(jnp.float32) / freq_dim))
        ...
        freqs = jnp.concatenate(
            [freqs_x, freqs_y], axis=-1).astype(jnp.float32)
        freqs = freqs[..., ::2]
