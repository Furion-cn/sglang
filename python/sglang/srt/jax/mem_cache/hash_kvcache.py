...
class ReqToHashKVCachePool(KVCache):
    ...
    def _create_cache(self):
        self.k_cache = [
            jnp.zeros(
                (self.max_seq_len*self.max_batch_size, self.head_num*self.head_dim), dtype=self.dtype)
    ...
    def set_kv_buffer(
        ...
        assert loc.shape[0] == cache_k.shape[0] == cache_v.shape[0]
        self.k_cache[layer_id] = self.k_cache[layer_id].at[loc].set(cache_k)
        self.v_cache[layer_id] = self.v_cache[layer_id].at[loc].set(cache_v)
