        ...
        try:
            ...
            if jax_key == "model.embed_tokens.weight":
                ...
                # 处理分离的 Q、K、V 权重 - 从 qkv_proj 中拆分
                    ...
                    return layer_weights.get("self_attn", {}).get("q_proj", {}).get("weight")
                elif jax_key.endswith("self_attn.k_proj.weight"):
                    return layer_weights.get("self_attn", {}).get("k_proj", {}).get("weight")
                elif jax_key.endswith("self_attn.v_proj.weight"):
                    return layer_weights.get("self_attn", {}).get("v_proj", {}).get("weight")
                elif jax_key.endswith("self_attn.q_proj.bias"):
                    return layer_weights.get("self_attn", {}).get("q_proj", {}).get("bias")
                elif jax_key.endswith("self_attn.k_proj.bias"):
                    return layer_weights.get("self_attn", {}).get("k_proj", {}).get("bias")
                elif jax_key.endswith("self_attn.v_proj.bias"):
                    return layer_weights.get("self_attn", {}).get("v_proj", {}).get("bias")
