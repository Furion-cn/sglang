...
import sglang.srt.jax.model_converter.converter_logging as converter_logging
from sglang.srt.jax.model_converter.multihost_dataloading import MultiHostDataLoadIterator
...
def create_orbax_checkpoint_manager(
  ...
  if not enable_checkpointing:
    converter_logging.log("Checkpointing disabled, not creating checkpoint manager.")
    return None
  converter_logging.log(f"Creating checkpoint manager with ocdbt={use_ocdbt} and zarr3={use_zarr3}")
  ...
  converter_logging.log("Checkpoint manager created!")
  return mngr
...
def create_orbax_emergency_checkpoint_manager(
  ...
  converter_logging.log("Creating emergency checkpoint manager...")
  ...
  converter_logging.log("Emergency checkpoint manager created!")
  return manager
...
def create_orbax_emergency_replicator_checkpoint_manager(
  ...
  flags.FLAGS.experimental_orbax_use_distributed_process_id = True
  converter_logging.log("Creating emergency replicator checkpoint manager...")
  ...
  converter_logging.log("Emergency replicator checkpoint manager created!")
  return manager
...
def process_replicator_error_file(error_file: str) -> bool:
  ...
  if error_file_path_exists:
    converter_logging.log(f"replicator_error_handler: file found: {error_file}.")
...
def read_replicator_error_file(error_file: str):
  ...
  try:
    ...
    converter_logging.log(f"Contents of replicator error file:\n{error_data}")
  except (OSError, ValueError) as e:
    converter_logging.log("replicator_error_handler: Failed to read contents of failed" f" file: {e}")
...
def cleanup_replicator_error_file(error_file: str):
  ...
  try:
    ...
    converter_logging.log("replicator_error_handler: Failed to remove replicator errors file:" f" {e}")


def print_save_message(step, async_checkpointing):
  if async_checkpointing:
    converter_logging.log(f"Started an asynchronous checkpoint save for step {step}")
  else:
    converter_logging.log(f"Saved a checkpoint at step {step}.")
...
def load_state_if_possible(
  ...
  if checkpoint_manager is not None:
    converter_logging.log("checkpoint manager exists so trying to load this run's existing checkpoint")

    step = checkpoint_manager.latest_step() if step < 0 else step
    if step is not None:
      converter_logging.log(f"restoring from this run's directory step {step}")
  ...
  if load_parameters_from_path != "":
    ...
    converter_logging.log(f"restoring full state from {load_full_state_from_path=}")
    ...
    converter_logging.log("No existing checkpoints found, not restoring checkpoint.")
    return None, None
...
def setup_checkpoint_logger(config) -> Any | None:  # pytype: disable=attribute-error
  ...
  converter_logging.log("Setting up checkpoint logger...")
  if config.enable_checkpoint_cloud_logger:
    ...
    converter_logging.log("Successfully set up checkpoint cloud logger.")
...
def load_params_from_path(
  ...
  converter_logging.log(f"restoring params from {load_parameters_from_path}")
