...
class _DeepEPDispatcherImplLowLatency(_DeepEPDispatcherImplBase):
    ...
    def dispatch_a(
        ...
        buffer = self._get_buffer()
        topk_idx = topk_idx.to(torch.int64)
        # expected_m=(bs*world_size*8+256)//256,
        # world_size 中的部分其实已经在 hidden_states.shape[0] 中，两者数值一样，但是含义上来讲，这里使用 bs*world_size 是最准确的，
        # world_size 中分多少到 bs 中取决于并行配置: attn_tp_size,dp
        # note: ep_size = tp_size, ep_size can be configured in the future. Currently, only support ep_size=tp_size
        # q: different ranks have different batch_size * seq_len
        # a: hidden_states has been gathered, the shape is [bs*world_size,7168]
        expected_m = (
    ...
    def _dispatch_core(
        ...
        """
        buffer = self._get_buffer()
        # packed_recv_hidden[0]=[num_local_experts, num_max_dispatch_tokens_per_rank * num_ranks, hidden]
        # packed_recv_hidden[1]=[num_local_experts, num_max_dispatch_tokens_per_rank * num_ranks, hidden // 128]
