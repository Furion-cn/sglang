...
class TestDeepEPMoE(unittest.TestCase):
    def setUp(self):
        ...
        # 初始化权重
        with torch.no_grad():
            self.model.w13_weight.data = torch.zeros(
                self.num_experts,
            ...
            for i in range(self.num_experts):
                for j in range(2 * self.intermediate_size):
                    for k in range(self.hidden_size):
                        self.model.w13_weight.data[i, j, k] = (
                            1 * (i + 1) * (j + 1) * (k + 1)
                        )

            self.model.w2_weight.data = torch.zeros(
                self.num_experts,
                self.hidden_size,
            ...
            for i in range(self.num_experts):
                for j in range(self.hidden_size):
                    for k in range(self.intermediate_size):
                        self.model.w2_weight.data[i, j, k] = (
                            1 * (i + 1) * (j + 1) * (k + 1)
                        )

    def test_forward_masked_with_runner(self):
        # 创建输入数据
        masked_hidden_states = torch.empty(
            self.num_experts,
            self.seq_len,
            self.hidden_size,
            device=self.device,
        ...
        normal_hidden_states = torch.empty(
            total_tokens, self.hidden_size, device=self.device, dtype=torch.bfloat16
        )

        # 准备 reorder_topk_ids 和 seg_indptr
        reorder_topk_ids = torch.zeros(
        ...
        for i in range(self.num_experts):
            for j in range(self.seq_len):
                if j < masked_m[i]:
                    for k in range(self.hidden_size):
                        masked_hidden_states[i, j, k] = (
                            1.0 * (i + 1) * (j + 1) * (k + 1)
                        )
                        normal_hidden_states[token_idx, k] = (
                            1.0 * (i + 1) * (j + 1) * (k + 1)
                        )
        ...
        # 打印调试信息
        print(
            f"masked_hidden_states shape: {masked_hidden_states.shape}, {masked_hidden_states=}"
        )
        print(
            f"normal_hidden_states shape: {normal_hidden_states.shape}, {normal_hidden_states=}"
        )
    ...
    def test_forward_integration(self):
        ...
        topk_idx = torch.randint(
            0, self.seq_len, (self.batch_size, 2), device=self.device
        )
        topk_weights = torch.rand(self.batch_size, 2, device=self.device)
        reorder_topk_ids = torch.randint(
            0, self.num_experts, (self.seq_len,), device=self.device
        )
        seg_indptr = torch.zeros(
            self.num_experts + 1, dtype=torch.int32, device=self.device
        )
        seg_indptr[1:] = torch.cumsum(
            torch.tensor([20, 42, 30, 36], dtype=torch.int32, device=self.device), dim=0
        )
        ...
        # 验证输出形状
        self.assertEqual(
            output.shape, (self.num_experts, self.seq_len, self.hidden_size)
        )
    ...
    def test_grouped_gemm_correctness(self):
        ...
        for i, length in enumerate(masked_m):
            ...
            self.assertTrue(
                torch.allclose(beyond_mask, torch.zeros_like(beyond_mask), atol=1e-5)
            )
